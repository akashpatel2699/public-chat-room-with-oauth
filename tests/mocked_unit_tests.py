...
from app import check_for_bot_command, check_for_valid_image, add_new_connected_user,\
            remove_disconnected_user, emit_all_messages, add_new_message,\
            on_connect, on_disconnect, on_new_message,\
            on_new_facebook_user, on_new_google_user
from bot import NAME, FUN_TRANSLATE_BASE_URL, OPEN_WEATHER_API_BASE_URL,PREDICT_AGE_BASED_ON_API
...
class UserObject:
    ...
    def order_by(self,created_at):
        return MessageObject("akashpatel",\
            "This is a message object",MessageType.TEXT_MESSAGE,\
...
class mocked_unit_tests(unittest.TestCase):
    def setUp(self):
        ...
        self.check_weather_bot_command_success= [
            {
                KEY_INPUT: "!! weather jersey city",
                KEY_EXPECTED: "67 F"
            }, {
                KEY_INPUT: "!! weather pluto",
                KEY_EXPECTED: "67 F"
            }
        ]
        ...
        self.check_predict_age_bot_command_failure= [
            {
                ...
                KEY_INPUT: "!! predict_age ",
                KEY_EXPECTED: "Try some other name than "
            }
        ]
    ...
    def test_check_for_image_url(self,my_mocked_request_dot_head):
        my_mocked_request_dot_head.side_effect = [HeaderObject("image/jpg"),HeaderObject("text/html")]
        ...
        self.assertEqual(success_response,self.check_for_valid_image_success[0][KEY_EXPECTED])
        self.assertEqual(failure_response,self.check_for_valid_image_failure[0][KEY_EXPECTED])
    ...
    @mock.patch("app.db.session.query",return_value=UserObject("akashpatel@gmail.com",'12345',\
        AuthUserType.FACEBOOK,"akashpatel","https://profileURL.com"))
    def test_add_new_user_that_exist_in_db(self,mocked_db_query,mocked_socketio_emit,mocked_db_session_commit,\
    mocked_models_Connected_users,mocked_db_session_add):
        for test_case in self.add_new_connected_user_success:
            input = test_case[KEY_INPUT]
            add_new_connected_user(
                input[KEY_CHANNEL],input[KEY_SOCKET_ID],input[KEY_NAME],input[KEY_EMAIL],
                input[KEY_AUTH_TYPE],input[KEY_PROFILE_URL]   
            )
            if input[KEY_SOCKET_ID] != "0000" or input[KEY_AUTH_TYPE].value != "google":
                mocked_socketio_emit.assert_called_with( input[KEY_CHANNEL],
                    {
    ...
    @mock.patch("app.db.session.commit",return_value=None)
    @mock.patch("app.socketio.emit",return_value=None)
    @mock.patch("app.db.session.query",return_value=UserObject("akashpatel@gmail.com",\
        '12345',AuthUserType.FACEBOOK,"akashpatel","https://profileURL.com"))
    def test_remove_disconnected_user(self,mocked_db_query,mocked_socketio_emit,mocked_db_session_commit,\
    mocked_db_session_delete):
        for test_case in self.remove_disconnected_user_success:
            input = test_case[KEY_INPUT]
            remove_disconnected_user(
                input[KEY_CHANNEL],input[KEY_SOCKET_ID]
            )
            mocked_socketio_emit.assert_called_with( input[KEY_CHANNEL],
                {
                'removeUser': "akashpatel"
                },
                include_self=False
            )
    ...
    @mock.patch("app.db.session.query",return_value=UserObject("akashpatel@gmail.com",\
        '12345',AuthUserType.FACEBOOK,"akashpatel","https://profileURL.com"))      
    def test_add_new_message(self,mocked_db_query,mocked_socketio_emit,\
    mocked_models_Messages,mocked_session_add,mocked_session_commit):
        for test_case in self.add_new_message_success:
            
            input = test_case[KEY_INPUT]
            add_new_message(
                input[KEY_CHANNEL],input[KEY_SOCKET_ID],input[KEY_MESSAGE],\
                input[KEY_MESSAGE_TYPE],input[KEY_NAME],input[KEY_EMAIL]
            )
            ...
            add_new_message(
                    input[KEY_CHANNEL],input[KEY_SOCKET_ID],input[KEY_MESSAGE],\
                    input[KEY_MESSAGE_TYPE],"",input[KEY_EMAIL]
            )
            mocked_session_commit.called_once()
    
    @mock.patch("app.user_authenticated")
    @mock.patch("app.flask.request")
    def test_on_connect(self,mocked_request_sid,mocked_user_authenticated):
        mocked_request_sid.sid = "1234"
        on_connect()
        mocked_user_authenticated.assert_called_with(AUTHENTICATED_CHANNEL,\
            False,mocked_request_sid.sid)
     
    @mock.patch("app.user_authenticated")
    @mock.patch("app.remove_disconnected_user")
    @mock.patch("app.flask.request")
    def test_on_disconnect(self,mocked_request_sid,mocked_remove_disconnected_user,\
    mocked_user_authenticated):
        mocked_request_sid.sid = "1234"
        on_disconnect()
        mocked_remove_disconnected_user.assert_called_with(REMOVE_DISCONNECTED_USER_CHANNEL,\
    ...
    @mock.patch("app.flask.request")
    def test_on_new_facebook_user(self,mocked_request_sid,mocked_add_new_connected_user,\
    mocked_emit_all_messages,mocked_user_authenticated):
        for test_case in self.on_new_facebook_user_success:
            ...
            mocked_add_new_connected_user.assert_called_with(ADD_NEW_USER_CHANNEL,mocked_request_sid.sid,\
            expected[KEY_NAME],expected[KEY_EMAIL],AuthUserType.FACEBOOK,expected['url'])
            mocked_emit_all_messages.called_once()
            mocked_user_authenticated.assert_called_with(AUTHENTICATED_CHANNEL,True,expected[KEY_SOCKET_ID])
    ...
    @mock.patch("app.flask.request")
    def test_on_new_google_user(self,mocked_request_sid,mocked_google_verify_token,\
    mocked_add_new_connected_user,mocked_emit_all_messages,mocked_user_authenticated):
        for test_case in self.on_new_google_user_success:
            ...
            mocked_add_new_connected_user.assert_called_with(ADD_NEW_USER_CHANNEL,mocked_request_sid.sid,\
            expected[KEY_NAME],expected[KEY_EMAIL],AuthUserType.GOOGLE,expected[KEY_PROFILE_URL])
            mocked_emit_all_messages.called_once()
            mocked_user_authenticated.assert_called_with(AUTHENTICATED_CHANNEL,True,expected[KEY_SOCKET_ID])
